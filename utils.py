            ...
            if y <= max(p1y, p2y):
                if x <= max(p1x, p2x):
                    ...
                    if p1x == p2x or x <= xinters:
                        inside = not inside
        print("p2x ", p2x, " p2y ", p2y, " x ",x, " y ", y, " xinters ", xinters, " inside ",inside," z ", z)
        p1x, p1y = p2x, p2y

    if height_constraints:
        lower, upper = min(height_constraints), max(height_constraints)
        print("inside: ",inside , " z: ", z, " lower: ", lower , " upper: ", upper)
...
def rectify_depth(depth, interpolate=True):
    ...
    if interpolate:
        # ARD: didn't have size check
        a = np.argwhere( rectified != 0.)
        if (a.size != 0):
